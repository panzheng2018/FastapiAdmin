<template>
  <div class="fa-full-height">
      ...
      >
        <template #left>
          <FaTableHeaderLeft
            ...
            :perm-create="['module_produce:produce_worder:create']"
            :perm-import="['module_produce:produce_worder:import']"
            :perm-export="['module_produce:produce_worder:export']"
            :perm-delete="['module_produce:produce_worder:delete']"
            :perm-patch="['module_produce:produce_worder:patch']"
    ...
    <FaImportDialog
      ...
      default-template-file-name="produce_worder_import_template.xlsx"
...
import ProduceWorderAPI, {
  type ProduceWorderForm,
  type ProduceWorderPageQuery,
  type ProduceWorderTable,
} from "@/api/module_produce/produce_worder";
...
type ProduceWorderSearchFormParams = {
  no?: string;
  component_id?: string;
  craft_id?: string;
  plan_count?: string;
...
const searchForm = ref<ProduceWorderSearchFormParams>({
  no: undefined,
  component_id: undefined,
  craft_id: undefined,
...
const { selectedRows, selectedIds, batchDeleting, onTableSelectionChange } =
  useTableSelection<ProduceWorderTable>();
...
const {
  ...
  core: {
    apiFn: ProduceWorderAPI.getProduceWorderList,
    apiParams: {
      page_no: 1,
      page_size: 10,
    },
    columnsFactory: (): ColumnOption<ProduceWorderTable>[] => [
      ...
      { prop: "no", label: "单号", minWidth: 80, showOverflowTooltip: true, headerAlign: "center" },
      {
        ...
        headerAlign: "center",
        formatter: (row: ProduceWorderTable) => row.project_name || "—",
      },
      {
        prop: "component_id",
        label: "所属部件",
      ...
      { prop: "updated_time", label: "更新时间", width: 168, sortable: true, showOverflowTooltip: true, headerAlign: "center", visible: false },
      {
        ...
        minWidth: 100,
        headerAlign: "center",
        formatter: (row: ProduceWorderTable) => row.created_by?.name ?? "—",
        visible: false,
      },
      {
        prop: "updated_by",
        ...
        minWidth: 100,
        headerAlign: "center",
        formatter: (row: ProduceWorderTable) => row.updated_by?.name ?? "—",
        visible: false,
      },
      {
        prop: "operation",
        ...
        align: "center",
        headerAlign: "center",
        formatter: (row: ProduceWorderTable) => formatOperationCell(row),
      },
    ],
  },
});
...
const importContentConfig = computed<IContentConfig>(() => ({
  permPrefix: "module_produce:produce_worder",
  cols: crudCols.value,
  indexAction: async () => ({}),
  importTemplate: () => ProduceWorderAPI.downloadTemplateProduceWorder(),
}));
...
const exportContentConfig = computed(() => ({
  ...
  exportsBlobAction: async (params: IObject) => {
    const merged = {
      ...
      ...params,
    } as unknown as ProduceWorderPageQuery;
    const res = await ProduceWorderAPI.exportProduceWorder(merged);
...
const detailFormData = ref<ProduceWorderTable>({});
...
const formData = ref<ProduceWorderForm>(createInitialFormData());
...
const crud = useCrudForm<ProduceWorderForm>({
  formData,
  initialFormData: createInitialFormData(),
  dialogVisible,
  dataFormRef,
  formRenderKey,
  detailApi: (id: number) => ProduceWorderAPI.getProduceWorderDetail(id),
  createApi: (data: ProduceWorderForm) => {
    ...
    return ProduceWorderAPI.createProduceWorder(payload);
  },
  updateApi: (id: number, data: ProduceWorderForm) => {
    const { project_id, ...payload } = data;
    return ProduceWorderAPI.updateProduceWorder(id, payload);
  },
...
function buildRowActions(row: ProduceWorderTable): TableOperationAction[] {
  const all: TableOperationAction[] = [
    {
      key: "detail",
      label: "详情",
      artType: "view",
      perm: "module_produce:produce_worder:detail",
    ...
    },
    {
      ...
      artType: "edit",
      icon: "ri:edit-2-line",
      perm: "module_produce:produce_worder:update",
    ...
    },
    {
      ...
      perm: "module_produce:produce_worder:delete",
      run: () => deleteRow(row),
    },
  ];
  return all;
...
async function handleBatchDelete() {
  ...
  try {
    ...
    await ProduceWorderAPI.deleteProduceWorder(ids);
    faTableRef.value?.elTableRef?.clearSelection();
    await refreshRemove();
  } catch {
    // 用户取消
...
async function runBatchStatus(value: "enable" | "disable") {
  ...
  try {
    ...
    await ProduceWorderAPI.batchProduceWorder({ ids, status });
...
async function handleCrudImportUpload(formData: FormData) {
  try {
    const res = await ProduceWorderAPI.importProduceWorder(formData);
