...
CREATE TABLE `produce_worder` (
  ...
  `plan_count` int NOT NULL DEFAULT 1 COMMENT '数量',
  `real_count` int NULL DEFAULT NULL COMMENT '实际数量',
  `plan_end_time` datetime NOT NULL COMMENT '完工时间',
  `real_end_time` datetime DEFAULT NULL COMMENT '实际完工时间',
  `plan_user_id` int NOT NULL COMMENT '执行用户',
  `real_user_id` int NULL DEFAULT NULL COMMENT '实际执行用户',
