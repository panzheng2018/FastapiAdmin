...
from .crud import ProduceWreportCRUD
from .schema import (
    ProduceWreportCreateSchema,
    ProduceWreportOutSchema,
    ProduceWreportQueryParam,
    ProduceWreportUpdateSchema,
)


class ProduceWreportService:
    """提报工单模块服务层"""
    ...
    @staticmethod
    def _populate_names(item: ProduceWreportOutSchema, obj: Any) -> ProduceWreportOutSchema:
    ...
    async def create(self, data: ProduceWreportCreateSchema) -> ProduceWreportOutSchema:
        ...
        obj = await ProduceWreportCRUD(self.auth, self.db).create(data=data)
    ...
    async def batch_import(self, file: UploadFile, update_support: bool = False) -> str:
        ...
        try:
            ...
            for i, row in enumerate(mapped_rows, start=1):
                try:
                    create_schema = ProduceWreportCreateSchema.model_validate(row)

                    exists_obj = await ProduceWreportCRUD(self.auth, self.db).get(no=create_schema.no)
                    if exists_obj:
                        if update_support:
                            await ProduceWreportCRUD(self.auth, self.db).update(id=getattr(exists_obj, 'id'), data=create_schema)
                            success_count += 1
                        else:
                            error_msgs.append(f"第{i}行: 单号 {create_schema.no} 已存在")
                        continue

                    await ProduceWreportCRUD(self.auth, self.db).create(data=create_schema)
                    success_count += 1
